...
require 'spec_helper'
require 'journey/fare_calculator'


RSpec.describe Journey::FareCalculator do
  it 'returns [] when journey has no sections' do
    calc = Journey::FareCalculator.new(sections: {}, alternatives: {})
    expect(calc.fares_for({})).to eq([])
  end

  it 'builds combinations and calculates total price and currency' do
    sections = {
      's1' => { 'alternatives' => ['a1', 'a2'] },
      's2' => { 'alternatives' => ['b1'] }
    }
    alternatives = {
    ...
    calc = Journey::FareCalculator.new(sections: sections, alternatives: alternatives)
    fares = calc.fares_for('sections' => ['s1', 's2'])
